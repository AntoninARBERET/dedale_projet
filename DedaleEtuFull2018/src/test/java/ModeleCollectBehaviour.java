 ...
 *
 */
public class ModeleCollectBehaviour extends DedaleSimpleBehaviour {
	...
	public ModeleCollectBehaviour(final CollectMultiAgent myagent) {
		super(myagent);
		...
		if (myPosition!=null){
			...
			if(previousPosition !=null && previousPosition.equals(myPosition)) {
				...
				if(myDedaleAgent.getBlockedSince()<2) {
					...
					myDedaleAgent.addBehaviour(new BlockingSendMessageBehaviour(myDedaleAgent, "-1", nextNode, myDedaleAgent.getTargetNode()));
				}
			}else{
